...
module.exports =
{
    ...
    async execute(client, message, args, Discord)
    {
        ...
        let botMessage = fs.readFileSync(`txt/${this.name}.txt`, 'utf8').replace(/{tournament}/g, tournament).replace(/{round}/g, round).replace(/{roundName}/g, roundName[round>=numRounds ? 0 : round]).replace(/{extraMessage}/g, extraMessage).replace(/{screenshots}/g, msg.guild.channels.cache.get('894372076884992015')); //msg.guild.channels.cache.get('849784629061746708'));
...
function errorMessage()
{
    const embedMessage = new dc.MessageEmbed()
        ...
        .addField('Required Syntax', `orsh tournament round numRounds tables (tableStart) (dayFinal)`, false)
